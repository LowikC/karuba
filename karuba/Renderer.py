...
def screen_to_grid(screen_position: ScreenPosition) -> Optional[GridPosition]:
    ...
    screen_x = screen_position.x - board_rect.x
    screen_y = screen_position.y - board_rect.y

    # x inside the tiles area
    if top_left_tiles.x <= screen_x <= top_right_tiles.x:
        x = screen_position.x - top_left_tiles.x
        x = x // int(cell_width)
    # x in left column
    elif top_left_tiles.x - 57 <= screen_x < top_left_tiles.x:
        x = 0
    # x in right column
    elif top_right_tiles.x < screen_x:
        x = BOARD_WIDTH - 1
    else:
        x = None

    # y in tiles area
    if top_left_tiles.y <= screen_y <= bottom_left_tiles.y:
        y = screen_position.y - top_left_tiles.y
        y = y // int(cell_height)
    # y in bottom row
    elif bottom_left_tiles.y <= screen_y <= bottom_left_tiles.y + 52:
        y = BOARD_HEIGHT - 1
    # y in top row
    elif 0 <= screen_y < top_left_tiles.y:
        y = 0
    else:
        y = None
